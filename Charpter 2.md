...
1.  BIOS
    *   开机后，post 开机自检
    *   寻找显卡和执行BIOS (此时, `CS : IP = 0xF000 : 0xFFF0`,  CS/IP 两个寄存器) 
...
中断来源于**外设**（键盘，鼠标，网卡，声卡，显卡），来自不同的硬件设备的计时器和网络的中断。
  处理时间：异步，不知道什么时候产生
  响应：对用户应用程序透明
...
1.  保存当前处理状态（寄存器之类的一些数据）--便于后续程序的继续执行
...
系统调用来源于**应用程序**，应用程序主动向操作系统发出服务请求。程序访问主要是通过高层次的API，而不是直接调用系统调用函数。
   ...
   处理时间：异步或同步（发出请求点是异步，返回的点可能是异步或同步）
   响应：等待和处理
系统调用接口---**APIs**
...
**用户态**：操作系统运行中，CPU处于的特权级别，特权级别低，不能直接执行特权指令
